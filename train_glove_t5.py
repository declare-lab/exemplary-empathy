...
from glove_models import ERGGloVeMainModel
...
def configure_optimizer(model, args):
    ...
    optimizer = optim.Adam(optimizer_grouped_parameters, **optimizer_kwargs)
    return optimizer
...
def train_or_eval_model(model, dataloader, optimizer=None, train=False, main_loss_w=1, empathy_loss_w=1, sentiment_loss_w=1):
    ...
    for conv_id, emotion, context, response, exemplars, empathy1_labels, empathy2_labels, empathy3_labels, sentiment in tqdm(dataloader, leave=False):
        ...
        losses.append(loss.item()) # change it to total_loss to early stop depending on the total_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--add-exemplars", default=None, help="Whether to use add exemplars.")
    ...
    parser.add_argument("--main-loss-w", type=float, default=1, help="loss weight for the generative loss")
    parser.add_argument("--empathy-loss-w", type=float, default=1, help="loss weight for the empathy loss")
    parser.add_argument("--sentiment-loss-w", type=float, default=1, help="loss weight for the sentiment loss")
    ...
    if args.inference is None:
        model = ERGGloVeMainModel(model_name, max_source_length, max_target_length, strategy, args.add_exemplars, args.max_exemplars, args.fixed).cuda()
        ...
        with open(f"saved/{run_ID}/logs.txt", "w") as f:
            f.write(str(args)+ "\n\n")
        ...
        for e in range(n_epochs):
            ...
            with open(f"saved/{run_ID}/logs.txt", "a") as f:
                f.write("Epoch {}: Train loss: {}, Valid loss: {}\n".format(e+1, train_loss, valid_loss))

    else:
        run_ID = args.inference
        
    model = ERGGloVeMainModel(model_name, max_source_length, max_target_length, strategy, args.add_exemplars, args.max_exemplars).cuda()
